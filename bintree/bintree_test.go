...
func TestCopyI(t *testing.T) {
	...
	bt2 := bt.CopyI()
	if bt.Size != bt2.Size {
		t.Errorf("After copyi size got %v expected %v", bt2.Size, bt.Size)
	}
	bt2.Print()
	bt3 := bt.CopyI()
	if bt.Size != bt3.Size {
		t.Errorf("After copyi size got %v expected %v", bt3.Size, bt.Size)
	}
	bt3.Print()
	_ = j
	_ = a
